...
سرور Ubuntu پایدار روی GitHub Actions: **داده ماندگار، هویت Tailscale ثابت (IP ثابت)، SSH ساده با رمز (کلید اختیاری)، داشبورد رمزدار**. معماری فعلی: **v6.25** (آخرین به‌روزرسانی: ۲۰۲۶-۰۹-۱۶).
...
## تغییرات مهم اخیر (v6.17 → v6.25)
- **v6.25** — بستن حفره‌ی اصلی «ربات کر»: `save.sh` قبل از خالی‌کردن توکن‌ها برای آرشیو، نسخه‌ی پایدار `/var/lib/hermes-guard/env.preblank` و sentinel `/run/hermes-env-blanked` می‌سازد و `trap` روی `INT/TERM/HUP` هم دارد؛ اگر ران وسط همان پنجره کشته شود، نگهبان توکن را برمی‌گرداند. `gateway_guard.sh` با `flock` از اجرای هم‌زمان مصون شد.
- **v6.24** — نگهبان مقیم اتصال تلگرام (`hermes-gateway-guard.timer`، هر ۹۰ ثانیه): به‌جای «پروسه زنده است؟» می‌پرسد «واقعاً `Connected to Telegram (polling mode)` لاگ شده؟». حالت خطرناکی را می‌گیرد که سرویس `active` است ولی `No messaging platforms enabled` لاگ کرده و ربات کاملاً بی‌صداست. هرگز به فایل‌های سشن دست نمی‌زند.
- **v6.23** — ری‌استارت خودکار gateway بعد از هر نصب/بازنصب Hermes + پاک‌سازی `__pycache__` (رفع `ImportError` ناشی از کد کهنه در حافظه).
