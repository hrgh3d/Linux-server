...
## ⏱️ دقیقه‌های Actions
- ریپو **public** است؛ مصرف را نسبت به محدودیت‌های پلن اکانت پایش کنید: Settings → Billing and plans → Usage → Actions.
- اگر به هر دلیل ایجاد Run متوقف شود، زنجیره با اولین تیک کرون (هر ۱۰ دقیقه) پس از رفع محدودیت خودکار از سر می‌گیرد و state از ریپوی خصوصی بازیابی می‌شود — بدون از دست رفتن داده.
- گزینه‌های پایدار در صورت نیاز: پلن پولی یا مهاجرت به VPS واقعی (کل state در ریپوی خصوصی است؛ مهاجرت بدون از دست رفتن داده ممکن است).
