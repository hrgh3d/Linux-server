...
TOK="$(grep -m1 'TELEGRAM_BOT_TOKEN=' "$ENVF" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '"'\''
\r' | xargs)"
if [ -z "${TOK:-}" ]; then echo "RESULT: NO_TOKEN"; exit 0; fi
echo "token_len: ${#TOK}"
ALLOWED="$(grep -m1 'TELEGRAM_ALLOWED_USERS=' "$ENVF" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '"'\'' \r' | cut -d, -f1)"
